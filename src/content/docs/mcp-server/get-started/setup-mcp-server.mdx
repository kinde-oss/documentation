...
description: "Learn how to set up the Kinde MCP Server with AI assistants like Cursor, Claude Desktop, Claude CLI (Claude Code), and VS Code to manage users, organizations, roles, and permissions directly from your IDE using natural language"
sidebar:
  order: 1
tableOfContents:
  maxHeadingLevel: 3
...
7. Select the scopes you want to grant:
    ...
    See all the available [scopes you can use with the Kinde MCP Server](/mcp-server/about-mcp-server/operations-and-scopes)
...
Claude CLI uses command-based MCP setup rather than editing `claude_desktop_config.json`.
...
2. In your terminal, run the command below to add the Kinde MCP server:
  ...
  ```bash
  claude mcp add --transport http --header "Authorization: Bearer YOUR_API_KEY" kinde https://YOUR_BUSINESS.kinde.com/mcp
  ```
