...
The Kinde MCP server provides access to the following data:
...
<Aside>
  You can only get data for organizations, you cannot create or update organizations with MCP server.
</Aside>
...
- An environment level API key is setup in Kinde, which forms part of the MCP server configuration that is copied into your AI client.
...
For detailed setup instructions, see the [setting up guide](/mcp-server/setting-up/mcp-server/).
