...
To upload logos per organization, see [Set unique branding for an organization](/design/brand/apply-branding-for-an-organization/).

## Manage global logo via the Kinde Management API

Requires an M2M access token with the `update:environments` scope. See [get an access token for the Kinde Management API](/developer-tools/kinde-api/access-token-for-api/).

### Add logo

```bash
curl --request PUT \
  --url '<YOUR_DOMAIN>/api/v1/environment/logos/<TYPE>' \
...
See the [Kinde Management API documentation](/kinde-apis/management#tag/environments/put/api/v1/environment/logos/{type}) for more details.
