...
Style hooks are [HTML data attributes](https://developer.mozilla.org/en-US/docs/Learn_web_development/Howto/Solve_HTML_problems/Use_data_attributes) that can be targeted using the [CSS attribute selector](https://developer.mozilla.org/en-US/docs/Web/CSS/Attribute_selectors), allowing you to apply styles directly in your custom CSS.
...
For your custom styles to apply correctly, make sure they load **after** the Kinde required CSS. See [Including the Kinde required CSS](https://www.notion.so/Styling-the-widget-174d97b0d20580c89f0cfbb0a7e0bbd0?pvs=21).

</Aside>

## Available style hooks

Nearly all elements in the widget have a style hook applied, allowing you to style it as needed. The best way to see which hooks are available is by using DevTools. See [Inspecting Components and Layouts in DevTools](https://www.notion.so/Styling-the-widget-174d97b0d20580c89f0cfbb0a7e0bbd0?pvs=21) for step-by-step instructions.
...
>
  <label
    class="kinde-control-label"
    data-kinde-control-label="true"
    for="credentials_email"
  >
		Email
	</label>
  <input
...
<div
  ...
  data-kinde-form-field-variant="select-text"
>
  <label data-kinde-control-label="true">
		Email
	</label>
  <input
    data-kinde-control-select-text="true"
...
For example, if you wanted to style a specific text input type, like “email”, “url”, “password”, etc, which belongs to the **Control select text** component, you can safely do that via targeting the “type” attribute:
...
Light and dark themes are managed in Kinde. See [Design brand experience defaults](https://docs.kinde.com/design/pages/design-your-welcome-pages/) for more details.
...
>
  <span 
	  class="kinde-button-text" 
	  data-kinde-button-text="true"
	 >
		Save
	</span>
</button>
