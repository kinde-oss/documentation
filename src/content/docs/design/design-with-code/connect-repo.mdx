...
Your directory structure should be in the format `pages/(kinde)/(<route>)/page.[ts/tsx]` where `<route>` is used to map your code to a rendered page in Kinde. For example `pages/(kinde)/(register)/page.tsx` will map to the register / sign up page in the authentication flow. For more information, see [Understand page customization](/design/design-with-code/understand-page-design/).
