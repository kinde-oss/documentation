...
The 'page' is the whole UI that is rendered by the browser. You can provide your own markup for everything that surrounds the Kinde widget. We give you a handy placeholder so you can tell us where you want the widget rendered. 
...
Important! If you are using React for templating, note this code is rendered on the server. This means client side functions for example, `useEffect`,  will not be available.
...
`rootDir` - the directory where Kinde should look for your custom code. Defaults to `kindeSrc` so that you can include Kinde code alongside existing projects. Change this if you wish the code to live in another directory.

`version` - based on the date when the API version was released. Any breaking changes will be released in a new API version.

The structure should be in the format `pages/(kinde)/(<route>)/page.[ts/tsx]` where `<route>` is used to map your code to a rendered page in Kinde. For example `pages/(kinde)/(register)/page.tsx` will map to the register / sign up page in the auth flow.

The `page.[tsx/ts]` function can be written in the templating languages as per above.
...
`(index)` - If someone visits the root domain for your business (https://<yourdomain>.kinde.com)

If you need to provide specific customization for a page not listed her, reach out and let us know
...
Sometimes additional information will need to be passed to Kinde from your page. You can use the page settings object for this
