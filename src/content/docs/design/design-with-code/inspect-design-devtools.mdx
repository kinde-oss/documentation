...
5. Click the **caret (▸) icon** next to the element to expand its child elements.

## Step 3: View settings

Once a component or layout is located in the HTML:

1. Select the highlighted element from **Step 2: Locate in HTML**.
2. In the **Styles** (or **Rules**) panel, the element’s CSS rules will be displayed, revealing the settings applied to it.
3. Look for `--kinde-[…]` instances to identify the settings specific to the component or layout.
...
4. To view state-specific settings:
