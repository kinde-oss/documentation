...
The widget refers to a specific form component that lives on the page, that consists of fields, labels, buttons, text strings with URLs, and the Kinde logo.

Most styles can be switched out using our suite of [CSS custom properties](/design/design-with-code/design-with-css/) but for more complex customizations, we have provided [style hooks](/design/design-with-code/design-with-style-hooks/) that you can use to completely replace everything.
