...
description: "Learn how Kinde page customization works—React templates, route mapping, page context, and environment bindings"
sidebar:
  order: 5
tableOfContents:
  maxHeadingLevel: 3
...
featured: false
deprecated: false
ai_summary: "Explains how Kinde page customization works for hosted authentication and account pages. Covers page structure as markup around the Kinde widget, supported templating options including React, Liquid, JavaScript, and TypeScript, GitHub repository layout under kindeSrc, and the kinde.json rootDir and version settings. Lists customizable page routes such as login, register, password, OTP, MFA, billing, early access, and error pages, plus the default fallback route. Documents the page default export, request and context objects, pageSettings bindings, connection switching context fields, and how to access environment variables with getEnvironmentVariable. Intended for developers building custom UI for Kinde-hosted pages."
---

The page is the full user interface rendered by the browser, including everything that surrounds the Kinde widget. You can provide your own markup to shape the page design. Use the widget placeholder to tell Kinde where to render the widget.
...
Kinde uses server-rendered JavaScript to compose custom pages and supports these templating options:
...
<Aside title="Server-side rendered React only">

If you use React for templating, note that this code is rendered on the server. Client-side functions such as `useEffect` are not available.

</Aside>

## Page structure in the Git directory

The code for your pages must live in a GitHub repository (other Git providers to be supported later). Correct directory structure is required for Kinde to run your design code.
...
The `kinde.json` file defines the config for all custom code in Kinde, including workflows and custom pages. A typical config file looks like this:
...
`rootDir` - the directory where Kinde will look for your custom code. Defaults to `kindeSrc` so that you can include Kinde code alongside existing projects. Change this if you want the code in another directory.

`version` - based on the date when the API version was released. Breaking changes are released in a new API version.

## Which pages can I customize?

All Kinde-hosted pages can be customized. URL route mapping determines which template to use.

If a specific mapped route does not exist, a special route called `(default)` is rendered. Set up the `(default)` page so all pages follow this design unless you override them with custom code.

<Aside type="warning">

Once you start customizing pages, you are responsible for maintaining them. Always apply custom code to pages in a non-production environment first.
...
- `(index)` - Landing page when visiting the root domain (e.g. `https://auth.your_business.com`).
...
Used in password-based authentication and recovery. Password entry always takes place on Kinde's hosted domain; only the visual appearance of these pages can be customized. Embedding the password entry screen on your own domain (without redirecting to Kinde) is not supported for security reasons.
...
Your `page.[ext]` file should contain a default export. The name does not matter, but the usual convention is to call it `Page`. It receives a single `event` argument: an object with two keys, `context` and `request`.
...
- `authUrlParams` contains useful info from the authorization request URL
  ...
  - `clientId` - the `client_id` query param from the auth URL (the application ID)
  - `redirectUri` - the `redirect_uri` query param from the auth URL
- `locale` - localization data
  - `isRtl` - whether the requested language should be rendered right to left
...
The top-level `context` object contains information about the page itself, such as page state and content.
...
The `pageSettings` object is required when you need to enable bindings that give your page access to additional Kinde features, such as environment variables or the fetch API. Without declaring these bindings in `pageSettings`, these features won't be available in your page code.
    ...
    <Aside>
    Bindings are explicitly opted in for performance reasons. By declaring only the bindings you need, you tell Kinde's runtime which internal features to initialize, avoiding unnecessary computation and keeping page render as fast as possible.
    </Aside>

## Connection switching context

On authentication pages, the `context` object includes the connections a user can switch to and the action needed to perform a switch. Use these to build a connection switcher — see [Add a connection switcher to a custom page](/design/customize-with-code/switch-connection/).

### `context.connections`

- `available` - an array of connections the current user can switch to. Each entry includes `id`, `friendlyId`, `name`, `connectionType` (`social`, `enterprise`, `credential`, or `other`), `credentialMethod`, `identityType`, `provider`, and `logoName`.

### `context.actions`

- `switchConnection` - the action used to switch connection. Contains `actionUrl`, `method`, and a `fields` map. Pass this object (JSON-encoded) to the `data-kinde-change-connection-action` attribute on your switch buttons.

### `context.session`

- `pipelineStepId` - the current pipeline step identifier. Pass it to the `data-kinde-change-connection-psid` attribute.
- `state` - the current authentication `state` value, when present.

### `context.auth` additions

- `activeConnectionId` - the connection the flow is currently using.
- `sessionConnectionId` - the connection originally requested for the session, if any.

## How to access environment variables in your page

You can access environment variables in your custom UI pages using the `getEnvironmentVariable` method. To enable this, add the `pageSettings` object with the environment binding to each custom page.

Follow these steps:

1. Set up your environment variables in **Environment > Settings > Env variables**.
