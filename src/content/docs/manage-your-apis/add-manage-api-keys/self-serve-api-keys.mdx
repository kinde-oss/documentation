...
- [Self-serve portal for orgs](/build/self-service-portal/self-serve-portal-for-orgs/)
- [Self-serve portal for users](/build/self-service-portal/self-serve-portal-for-users/)
...
If you're not using a Kinde SDK, call the Account API endpoint and pass `subnav` or `sub_nav` to deep link to API Keys. See [Get self-serve portal link](https://docs.kinde.com/kinde-apis/frontend/#tag/self-serve-portal/get/account_api/v1/portal_link) for supported values.
...
- Use `subNav` (camelCase) in SDK helpers and either `sub_nav` (snake_case) or `subnav` (lowercase) in Account API requests.
