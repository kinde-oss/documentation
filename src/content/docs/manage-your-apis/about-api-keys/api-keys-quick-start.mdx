...
title: API keys quick start
description: "Register an API, create and verify API keys in minutes—then optionally connect MCP clients with scoped keys"
sidebar:
  order: 2
relatedArticles:
...
featured: false
deprecated: false
ai_summary: "Beginner quick start for API key management in Kinde. Walks through registering an API with a name and audience, creating a user or organization API key with scopes in the admin UI, and verifying keys in your application by calling the Kinde Management API verify endpoint with an M2M access token. Includes a JavaScript verification example that checks is_valid and scopes before serving a request, plus a sample verification response with key_id, org_code, and user_id. Also outlines an optional MCP path: upload an OpenAPI spec, create an MCP connection, issue a scoped user API key, and use Quick start snippets in an MCP client. Intended for developers and admins protecting their own APIs with Kinde API keys."
---
...
## Optional: Connect an MCP client to your API

If you want AI assistants to call your API through the Model Context Protocol instead of (or as well as) direct HTTP:

1. Upload an OpenAPI spec for your API.
2. Create an MCP connection and authorize the API.
3. Issue a user API key with the scopes (tools) you want available.
4. Paste the connection URL and key into your MCP client using the **Quick start** snippets in Kinde.

Full walkthrough: [Add a new connection](/mcp-servers/add-mcp-connection/).
