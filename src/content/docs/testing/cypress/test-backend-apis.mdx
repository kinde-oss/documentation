...
In this guide, we'll show you how to test backend APIs with Cypress. We will use Kinde refresh tokens for maintaining authenticated sessions in automated tests, including token rotation. Learn more about [testing backend APIs with Kinde](/testing/testing-backend-apis/).
