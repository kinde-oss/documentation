...
2. Create a setup file to refresh tokens before tests run:
...
3. Update the `playwright.config.ts` file to add the following:
    ...
    export default defineConfig({
      ...
      projects: [
        // Setup project - refreshes tokens
        {
          name: 'setup',
          testMatch: /api\/.*\.setup\.ts/,
        },

        // Main tests - use refreshed tokens
        {
          name: 'api-tests',
