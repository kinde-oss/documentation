...
When a user authorizes against an organization in Kinde their roles are returned as an array of IDs in the `roles` claim of the `access_token`.

Our SDKs commonly ship with a `getClaim` method to help with this.

For example in React you could use something like:
