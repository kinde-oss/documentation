...
### Re-**imports will not update user info**
...
- User details and passwords need to be in UTF-8 encoded CSV format (except for Auth0 imports, see below)
- File size limit 50MB

## Before you import users

- [Enable password authentication](https://kinde.com/docs/authentication-and-access/set-up-user-authentication/) before importing passwords
...
**Note: Importing users from MS Azure AD** Set up the [Azure AD connection in Kinde](/authenticate/enterprise-connections/azure/) before you import your users. Then when you import, Kinde will match users to the relevant connection based on their email address.
...
5. Most import errors can be fixed by editing the CSV file and then re-importing into Kinde. Any records that have already been imported, will be ignored.
