...
- **Kinde Management API** — call [Set user password](/kinde-apis/management#tag/users/put/api/v1/users/{user_id}/password) with `hashing_method: firebase-scrypt` and the same fields. PBKDF2-specific fields (`iterations` and `variant`) are rejected for Firebase scrypt, and the Firebase scrypt fields are rejected for any other hashing method.
