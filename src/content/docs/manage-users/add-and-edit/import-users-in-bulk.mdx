...
featured: false
deprecated: false
ai_summary: "Guide to importing users in bulk to Kinde using UTF-8 CSV or NDJSON files up to 49MB. Covers file requirements, username case-insensitivity, and preparing data so users link to roles, permissions, organizations, feature flags, properties, identities, and API scopes. Includes the full NDJSON schema with single-line and readable examples, plus CSV preparation covering required fields (email or phone, external_organization_id), optional user data, and password columns for hashed_password, hashing_method (crypt, bcrypt, sha256, md5, wordpress), salt, salt_position, and salt_format. Explains assigning users to multiple organizations with different roles across separate lines or a follow-up file. Provides a high-level overview of migrating from Firebase Authentication with a link to the dedicated Firebase guide, and points Auth0 users to the Auth0 import guide. Documents the import steps in the Kinde admin, error handling and re-import behavior, impact on end-users, weak password handling, and why re-imports do not overwrite existing records. Intended for admins and developers managing large-scale user migrations."
---
