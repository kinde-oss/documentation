...
Kinde does not currently have any Bot Detection systems (apart from what is provided via AWS) as our customers largely use passwordless authentication options or are using MFA. It is on our product roadmap to add this in future.

## Brute force protection

Brute force lockouts are applied after 5 unsuccessful sign in attempts made via an incorrect passwordless code (OTP), password, or multi-factor authentication code.

We also use blowfish salting and adaptive hashing algorithms, which prevents brute-forcing and time-based attacks. We will continue to evolve our methods to ensure strong security.
