...
Kinde holds an  ISO 27001:2022 certification, has an attestation for SOC 2 Type 2 and HIPAA, and are GDPR compliant. Please refer to our [Compliance certifications](/trust-center/privacy-and-compliance/compliance/) page for more information. A HIPAA BAA is available upon request.

## Data residency

Data is stored in the region you selected during sign up of our product. Currently the available regions are Sydney, Australia; Dublin, Ireland; Oregon, USA; London, UK; and Quebec, Canada. The only data replicated between regions is backend business metadata to facilitate the operation of the production services. Otherwise, all user data is not replicated between the regions and will remain in it’s original region.
...
Internal identity is generally managed through the company’s internal identity and access management (IAM) tool. All systems are connected to the company IAM platform via single sign-on (SSO) where supported. Multi-factor authentication is enforced for all users and audit logs are enabled and monitored for systems connected to the company SSO. For systems that do not support SSO, employees are required to use the company provided password manager to generate and manage secure credentials as well as enable MFA for added protection.
...
Kinde completes a network and web application penetration test at least annually, which was scoped for anything and everything related to Kinde. The test included common OWASP techniques and specifically targeted workflows such as privilege escalations, authentication bypasses, and cloud security. 
