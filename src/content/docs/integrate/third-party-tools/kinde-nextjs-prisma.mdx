...
1. Install Prisma dependencies by entering the following command in your project terminal:
    ...
    <PackageManagers pkg='@prisma/client dotenv' />
...
3. Open the `prisma.config.ts` file and add the following on top of the config file. This will allow Prisma to load environment variables from the `.env` file.    
...
4. Generate the Prisma Client by running the following command in your terminal.
...
5. Push the schema changes to your database by running the following command.
...
6. Create a new `db.ts` file in the `/src` directory by running the following:
    
    ```bash
    touch src/db.ts
    ```
    
7. Open the `db.ts` file and copy the following code into it. Save the file after adding the code:
...
4. Open the `.env` or `.env.local` file in your project and update the `KINDE_POST_LOGIN_REDIRECT_URL` field with the following value:
