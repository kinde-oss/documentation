...
export async function GET() {
  ...
  if (decodedToken) {
    ...
    return NextResponse.redirect(`${process.env.KINDE_SITE_URL}/api/auth/login?${params.toString()}`, 302);
  }
}
```
...
When you redirect the user to the endpoint `/ap/multipass`, this will generate the Multipass token and redirect the user logged in to your Shopify site.
