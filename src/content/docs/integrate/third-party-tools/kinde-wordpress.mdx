...
2. In the **Quick setup** section, enter your Kinde OpenID configuration URL and select **Load Configuration**.
    ...
    ```
    <Aside>
    Replace `<YOUR_DOMAIN>` with your [custom domain](/build/domains/pointing-your-domain/) (e.g., `https://auth.yourbusiness.com`) or your Kinde domain (e.g., `https://your_business.kinde.com`).
    </Aside>
...
5. If auto-configuration fails, manually enter the following details:
    ...
    <Aside>
    You can find all these endpoints by visiting the OpenID configuration URL (for example, `<YOUR_DOMAIN>/.well-known/openid-configuration`).
    </Aside>
