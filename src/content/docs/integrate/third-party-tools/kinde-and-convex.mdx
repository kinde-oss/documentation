...
2. Update the `users.ts` file to add the following functions:
...
3. Create a new webhook file and add the following contents:
    ...
    try {
        ...
        const decoded = await decodeWebhook(jwt, "https://tamalkinde.kinde.com");
...
4. Create a new http file and add the contents to handle the route:
