...
Convex already gets the signed-in user’s Kinde ID from the token `subject` claim, and you can read profile details (e.g., `givenName`, `familyName`, `email`) from the `useKindeAuth` hook’s `user` object. To use that data in queries, mutations, and across your app, store it in Convex. The steps below add a `users` table and a mutation that syncs the current user on sign-in.
