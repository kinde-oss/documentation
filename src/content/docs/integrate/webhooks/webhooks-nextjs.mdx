...
2. Install the localtunnel package with the following command:
...
3. Open a new terminal window and start the localtunnel server with the following command:
...
1. In Kinde, go to **Environment > Settings > Webhooks**
...
1. Create a new user or update an existing one in Kinde.
2. Switch back to the terminal where your server is running and you should see the data in your server console.

    ```bash
    POST /api/webhook 200 in 1680ms
    {
      user: {
        email: null,
        first_name: 'test',
