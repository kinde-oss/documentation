...
There are two methods for getting an access token.

### Method 1: Get a test access token

Follow the [Get token to test the Kinde Management API](/developer-tools/kinde-api/kinde-api-test-token/) guide to generate a test token.
