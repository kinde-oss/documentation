...
## Step 2: Authorize the application for relevant APIs

1. On the left, select **APIs** 
2. Select the three dots menu next to the relevant API, then choose Authorize application.
3. If you are using scopes, select the three dots menu next to an authorized API, then choose Manage scopes. For maximum security only enable the minimum scopes you require.
4. Next (optional): if this application is authorized for calling the Kinde management API [Get an access token for Kinde’s management API](/developer-tools/kinde-api/get-access-token-for-connecting-securely-to-kindes-api/).
