...
There are two main approaches to E2E testing with Kinde:

1. **API Testing with Refresh Tokens** – Bypass the UI login flow and use pre-authenticated tokens
2. **Full UI Testing** – Automate the complete sign-in flow through the browser

We recommend combining both approaches: use stored authentication state for most tests, and validate the actual login flow in dedicated auth tests.
...
### Step 1: Create a test user in Kinde

1. Go to your Kinde dashboard → **Users**
2. Create a dedicated test user (e.g., `e2e-test@yourcompany.com`)
...
## Approach 2: Testing the Full Login Flow

For tests that specifically validate authentication flows:

```tsx
// tests/auth-flow.spec.ts
import { test, expect } from '@playwright/test';

// Don't use saved auth state for these tests
...
## Approach 3: API Testing with Token Refresh
...
## Handling Email OTP (Passwordless) Flows

If your app uses passwordless authentication with email OTP, you'll need to capture the verification code. Here's an approach using Mailosaur:
...
### Alternative: Email-to-S3 Pipeline

If you have your own email infrastructure:
...
│   ├── auth-flow.spec.ts         # Login/logout UI tests
...
- [Testing Authentication Flows](/developer-tools/testing/testing-authentication-flows/) - Learn how to test sign-up and sign-in flows
