...
If you're using the [Kinde-J2EE](#logout-with-kinde-j2ee) servlets or the [Spring Boot starter](#logout-with-spring-boot), logout is handled for you.
...
export KINDE_LOGOUT_REDIRECT_URI=http://localhost:< replace with port of application server > # where users land after signing out
...
KINDE_LOGOUT_REDIRECT_URI=http://localhost:< replace with port of application server > # where users land after signing out
...
Set `kinde.oauth2.post-logout-redirect-uri` to sign users out of both your app and Kinde. You can set it directly, or read it from the `KINDE_LOGOUT_REDIRECT_URI` environment variable:
...
In Spring Boot starter versions 3.0.1 and earlier, the servlet (non-reactive) auto-configuration reads the redirect URL from the `KINDE_LOGOUT_REDIRECT_URI` environment variable or `.env` entry instead of the property. Set both to the same value. If you are upgrading from version 2.x, this property was previously named `okta.oauth2.post-logout-redirect-uri`.
