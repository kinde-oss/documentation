...
Provide a logout redirect URI, so Kinde knows where to send the user after they sign out. This is required to use [`logout`](#log-out).

```shell
export KINDE_LOGOUT_REDIRECT_URI=http://localhost:8080
```

Both URLs must also be added to your application in Kinde. Go to **Settings > Applications > [your app] > View details** and add them under **Allowed callback URLs** and **Allowed logout redirect URLs**, then select **Save**.
...
export KINDE_LOGOUT_REDIRECT_URI=<replace> # where users land after signing out
```
...
KINDE_LOGOUT_REDIRECT_URI=<replace>
KINDE_SCOPES=openid
```
...
Logging out ends the user's session in your app and on Kinde, then redirects them to your logout redirect URL.

<Aside>

This section is for the Kinde Java core. If you're using [Kinde-J2EE](#logout-with-kinde-j2ee) or the [Spring Boot starter](#logout-with-spring-boot), see those sections instead.

</Aside>

### Set the logout redirect URL

1. In Kinde, go to **Settings > Applications** and select **View details** on your app.
2. Add your URL under **Allowed logout redirect URLs**, for example `http://localhost:8080`.
3. Select **Save**.
4. Set the same URL in your app, either with the `KINDE_LOGOUT_REDIRECT_URI` environment variable or `.env` entry:

   ```shell
   KINDE_LOGOUT_REDIRECT_URI=http://localhost:8080
   ```

   Or programmatically, with the `KindeClientBuilder`:
...
If no logout redirect URL is configured, `logout()` throws an exception.

<Aside type="warning">

The URL in your app must exactly match one of your **Allowed logout redirect URLs**, including any trailing slash. For example, `http://localhost:8080` and `http://localhost:8080/` are treated as different URLs. If they don't match, Kinde ends the session but shows its default logged-out page instead of redirecting to your app.

</Aside>

### Generate the logout URL

`logout()` returns the Kinde logout URL, with your logout redirect URL added as a parameter. Clear your app's session, then redirect the browser to it.

```java
KindeClientSession kindeClientSession = kindeClient.clientSession();
AuthorizationUrl logoutUrl = kindeClientSession.logout();

// End the session in your app
HttpSession session = req.getSession(false);
if (session != null) {
    session.invalidate();
}

// End the session on Kinde
resp.sendRedirect(logoutUrl.getUrl().toString());
```

Kinde ends the user's session and redirects them to `KINDE_LOGOUT_REDIRECT_URI`.
...
The logout process generates the Kinde logout URL, with your configured `KINDE_LOGOUT_REDIRECT_URI` added. Redirect the browser to it to end the user's session on Kinde. Kinde then sends the user to the logout redirect URL. Throws an exception if no logout redirect URL is configured. See [Log out](#log-out).
...
### Logout with Kinde-J2EE

With `KindeLogoutServlet` mapped to `/logout` as above, a `GET` request to `/logout`:

1. Removes the Kinde tokens and user from the HTTP session, and invalidates it.
2. Redirects the browser to Kinde to end the user's Kinde session.
3. Kinde then redirects the user to `KINDE_LOGOUT_REDIRECT_URI`.

Link to it from your app, for example:
...
`KindeLogoutServlet` only supports `GET` and has no CSRF protection, so another site can sign your users out by linking to or embedding `/logout`. This can't expose user data, but take it into account when deciding how to expose the endpoint.

</Aside>

`KINDE_LOGOUT_REDIRECT_URI` must be set (see below). If it isn't, `/logout` fails with a server error and the user stays signed in to Kinde. The URL must also exactly match one of your **Allowed logout redirect URLs** in Kinde. If it doesn't, Kinde ends the session but shows its default logged-out page instead of redirecting to your app.
...
export KINDE_CLIENT_SECRET=<replace> # the secret used to authenticate the client against Kinde
export KINDE_REDIRECT_URI=http://localhost:8080/kinde-j2ee-app/login
export KINDE_LOGOUT_REDIRECT_URI=http://localhost:8080/kinde-j2ee-app # where users land after signing out
```
...
KINDE_CLIENT_SECRET=<replace>
KINDE_REDIRECT_URI=http://localhost:8080/kinde-j2ee-app/login
KINDE_LOGOUT_REDIRECT_URI=http://localhost:8080/kinde-j2ee-app
```
...
kinde:
  oauth2:
    ...
    post-logout-redirect-uri: http://localhost:< replace with port of application server >
```

### Logout with Spring Boot

Set `kinde.oauth2.post-logout-redirect-uri` in `application.yaml` to sign users out of both your app and Kinde. The Spring Boot starter reads the logout redirect URL only from this property, not from a `KINDE_LOGOUT_REDIRECT_URI` entry in a `.env` file.

To set it per environment, use a Spring placeholder. This reads the `KINDE_LOGOUT_REDIRECT_URI` environment variable if it's exported, and otherwise uses the default after the colon:

```yaml
kinde:
  oauth2:
    post-logout-redirect-uri: ${KINDE_LOGOUT_REDIRECT_URI:http://localhost:8080}
```

When this property is set, a `POST` request to `/logout`:

1. Clears the user's Spring Security session.
2. Redirects the browser to Kinde's logout endpoint, with the user's ID token and your post-logout redirect URL.
3. Kinde ends the user's session and redirects them to `post-logout-redirect-uri`.

A relative value, such as `/goodbye`, is resolved against your app's base URL. The resolved URL must exactly match one of your **Allowed logout redirect URLs** in Kinde, including any trailing slash.

Spring Security requires a CSRF token on `/logout`, so trigger logout from a form rather than a link. For example, with Thymeleaf:
...
If `post-logout-redirect-uri` is not set, `/logout` only clears the local Spring session. The user is still signed in to Kinde, so they will be signed straight back in the next time they log in.

</Aside>

<Aside>

In Spring Boot starter versions 3.0.1 and earlier, the servlet (non-reactive) auto-configuration reads the redirect URL from the `KINDE_LOGOUT_REDIRECT_URI` environment variable or `.env` entry instead of the property, and fails to start if it isn't set. On these versions, set both to the same value. If you are upgrading from version 2.x, this property was previously named `okta.oauth2.post-logout-redirect-uri`.
