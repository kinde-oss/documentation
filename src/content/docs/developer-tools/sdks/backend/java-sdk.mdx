...
## Kinde Core
The Kinde Java Core provides the core of the Kinde SDK for the Java Platform.
...
It is possible to retrieve the user information using an AccessToken. In the example below an access token is passed to init a client session.
...
The Management API provides a means to access the Kinde Management endpoints. This enables the development of features to control an application or organizations permissions via bespoke code and features rather than through the admin interface.
