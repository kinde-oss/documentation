...
- A [TanStack Start project](https://tanstack.com/start) using `@tanstack/react-router` and `@tanstack/react-start` `v1.132.25` or higher
...
1. Create `src/routes/api.auth.$.ts`:
...
1. Create `start.ts` in the project root 
...
2. Add the following code:
    ...
    import { createKindeGlobalMiddleware } from "@kinde/tsr/server"
...
1. In `src/routes/__root.tsx`, wrap the app so hooks and token refresh work:
    ...
    import { KindeTanstackProvider } from "@kinde/tsr/client"
...
Use `useKindeAuth` hook for authentication utility and components:
...
### 9. Display logged-in user information
