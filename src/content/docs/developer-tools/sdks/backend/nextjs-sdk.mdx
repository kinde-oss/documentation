...
1. Setup the the Kinde Management API please see [@kinde/management-api-js](https://github.com/kinde-oss/management-api-js)
2. In your application source code, create a folder at the top level called `scripts`.
3. Within that folder, create a file called `add-urls-to-kinde.mjs` and add the following code:

   ```jsx
   import { init, Callbacks } from '@kinde/management-api-js'

async function addLogoutUrlToKinde() {
  try {
    await Callbacks.addLogoutRedirectUrLs({
      appId: process.env.KINDE_CLIENT_ID,
      requestBody: {
        urls: [`https://${process.env.VERCEL_URL}`]
      }
    })
    console.log(`SUCCESS: Logout URL added to Kinde: ${process.env.VERCEL_URL}`)
  } catch (error) {
...
async function addCallbackUrlToKinde() {
  try {
    await Callbacks.addRedirectCallbackUrLs({
      appId: process.env.KINDE_CLIENT_ID,
      requestBody: {
    ...
    console.log(`SUCCESS: Callback URL added to Kinde: ${process.env.VERCEL_URL}/api/auth/kinde_callback`)
  } catch (error) {
...
;(async () => {
  if (process.env.VERCEL == 1) {
    try {
      init()
      await addCallbackUrlToKinde()
      await addLogoutUrlToKinde()
    } catch (error) {
