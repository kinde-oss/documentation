...
> If you’re using the pages router see [NextJS Pages Router](/developer-tools/sdks/backend/nextjs-prev-sdk/)
...
| [`refreshTokens`](#refreshtoken)                | Refresh tokens to get up-to-date Kinde data           |
...
const {getAccessToken} = getKindeSeverSesion();
...
```JSON
{
  ...
  "aud": [
    "feb8e697b967466eacb96d26c5ca0e12"
  ],
...
```jsx
"use client";
import {getKindeBrowserClient} from "@kinde-oss/kinde-auth-nextjs";

const {isAuthenticated} = getKindeBrowserClient();
...
const {user, getUser} = getKindeBrowserClient();
...
const {organization, getOrganization} = getKindeBrowserClient();
...
const {userOrganizations, getUserOrganizations} = getKindeBrowserClient();
...
const {getPermission} = getKindeBrowserClient();
...
const {permissions, getPermissions} = getKindeBrowserClient();
...
const {getFlag} = getKindeBrowserClient();
...
const {getBooleanFlag} = getKindeBrowserClient();
...
const {getIntegerFlag} = getKindeBrowserClient();
...
const {getStringFlag} = getKindeBrowserClient();
...
const {refreshData} = getKindeBrowserClient();
...
const {accessToken, getAccessToken} = getKindeBrowserClient();
...
const {accessTokenRaw, getAccessTokenRaw} = getKindeBrowserClient();
...
const {idToken, getIdToken} = getKindeBrowserClient();
...
const {idTokenRaw, getIdTokenRaw} = getKindeBrowserClient();
...
const {user, isLoading} = getKindeBrowserClient();
...
const {user, isLoading, error} = getKindeBrowserClient();
