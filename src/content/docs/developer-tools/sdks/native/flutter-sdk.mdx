...
2. **Configure authentication redirect**

    To ensure your app redirects correctly after authentication, confirm the following is set up correctly. 
    
    Check your `android/app/build.gradle` file to find your `minSdkVersion`. 

        <Tabs>
        <TabItem label="minSdkVersion >= 30">

        If your `minSdkVersion` is 30 or higher, ensure your main Activity in `AndroidManifest.xml` does not have an empty `taskAffinity` attribute. 
        ...
        If you're on `minSdkVersion >= 30` and you need to have a `taskAffinity` attribute, follow the instructions for `minSdkVersion < 30`.
        ...
        If your `minSdkVersion` is less than 30, do the following:

         1. Set the `taskAffinity` attribute of your main Activity in `AndroidManifest.xml` to a custom value. 
              ...
              <application
                  <activity
                      ...
                      android:exported="true"
                      android:launchMode="singleTop"
              +        android:taskAffinity="<your_custom_task_affinity>"
                  >
                      <!-- ... -->
                  </activity>
              ...
              This value must be **entirely different** from your app's namespace specified in the `android` block in the `android/app/build.gradle` file (e.g., `com.example.redirect` if your namespace is `com.example.app`).
             
              </Aside>


         2. Add the `net.openid.appauth.AuthorizationManagementActivity` activity to your manifest and set its `taskAffinity` to the same value as that of your main activity.
        ...
        <details>
        <summary>Why do we need this configuration?</summary>
        Flutter protects against the Android [StrandHogg vulnerability](https://developer.android.com/privacy-and-security/risks/strandhogg) by setting an empty `taskAffinity` but the empty `taskAffinity` prevents the authentication browser from redirecting to the app.
 
        Android SDK versions 30 and higher are safe from this vulnerability and so apps with `minSdkVersion >= 30` can safely remove the empty `taskAffinity` attribute.

        For apps with `minSdkVersion < 30`, a custom `taskAffinity` attribute is required to mitigate this vulnerability.
        </details>
...
You can pass optional parameters to sign users up for a billing plan or subscription through the login() or register() functions:

```dart
await sdk.login(
  type: AuthFlowType.pkce,
  additionalParameters: AdditionalParameters(planInterest: 'pro_monthly')
);

await sdk.register(
  additionalParameters: AdditionalParameters(
    planInterest: 'pro_monthly',
    pricingTableKey: 'spring_promo'
  )
);
```
...
The Flutter SDK provides the `generatePortalUrl` method to generate a one-time link to the Kinde self-serve portal. This allows users to manage their profile, billing, and organization settings.

### Generate a portal URL

```dart
final portalUrl = await sdk.generatePortalUrl();
```

### With subnav parameter

Direct users to a specific portal section using the `subnav` parameter:

```dart
final portalUrl = await sdk.generatePortalUrl(
  subnav: PortalUrlSubnav.organizationDetails,
);
```

### With return URL

Specify where users should return after exiting the portal:
...
Optional parameters that can be passed to authentication methods for billing and organization flows.
...
- `planInterest` (String?) - Pre-selects a billing plan for the user
- `pricingTableKey` (String?) - Specifies which pricing table to display
...
sdk.createOrg(
  orgName: "org_1234",
  planInterest: "enterprise_annual", // optional
  pricingTableKey: "enterprise_2024" // optional
);
```

**Note:** This method accepts `planInterest` and `pricingTableKey` as direct named parameters, unlike `login()` and `register()` which use the `additionalParameters` wrapper.
...
Generate a one-time link to the self-serve portal.

**Arguments:**

```dart
subnav?: PortalUrlSubnav
returnUrl?: "string"
```
...
```dart
sdk.generatePortalUrl();
sdk.generatePortalUrl(subnav: PortalUrlSubnav.organizationDetails);
sdk.generatePortalUrl(returnUrl: "https://yourapp.com/dashboard");
```

**Sample output:**

```dart
"https://yourapp.kinde.com/portal/..."
```
