...
await sdk.login(
  ...
  additionalParams: AdditionalParameters(planInterest: 'pro_monthly')
);
...
**Note:** Unlike `login()` and `register()`, this method accepts billing parameters as direct named parameters rather than through `AdditionalParameters`.
