...
5. Go to the **Details** page and copy the **Domain**, and **Client ID**. We will need it to configure our project
...
5. Copy the **Domain**, and **Client ID** to the following values of the `.env` file and save changes.
...
Kinde client provides login and register methods you can implement to your SPA.
...
To create a new organization within your application, you will need to run a similar function below.
    ...
    ```javascript
    document.getElementById("createOrganization")
      .addEventListener("click", async () => await kinde.createOrg();});
    ```
