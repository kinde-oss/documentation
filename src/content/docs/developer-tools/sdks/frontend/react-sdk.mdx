...
4. Import the `KindeProvider` component and wrap your application in it.
    ...
    Replace the `<your_kinde_client_id>` and `<your_kinde_domain>` placeholders in the code block below with the values from the **Details > App keys** section.
...
The token attribute has changed to camelCase.
