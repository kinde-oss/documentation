...
5. On the **Quick start** page, select **Set** for both callback URL and logout URL. This sets `http://localhost:3000` as the default for both URLs. To use a different URL, change it on the **Details** page.
6. To manually set the callback URLs, go to the **Details** page and scroll down to the **Callback URLs** section. Enter the following:
    ...
    <Aside>
    Replace `http://localhost:3000` with your server domain when you deploy to production. You can add multiple callback URLs. Add each URL on a new line.
    </Aside>
...
1. Download the [React starter kit](https://github.com/kinde-starter-kits/react-starter-kit)
...
4. Copy the environment variables from the **Quick start** page to the `.env` file and save changes.
...
8. Go to your **Kinde dashboard > Users** to find your newly created user.
...
2. Import the `KindeProvider` component and wrap your application in it.
    ...
    Replace the `<your_kinde_client_id>` and `<your_kinde_domain>` placeholders in the code block above with the values from **Details > App keys**.

See the sections below to configure your app.
...
The SDK also includes a `useKindeAuth` hook with `login` and `register` methods to start the flow.
...
- `loginHint` - prepopulates a user's email address on the sign-up and sign-in screens.
- `lang` - if you offer multi-language support, Kinde automatically selects the best language for your user based on their browser. To force a language and override the user's preference, pass this attribute.
...
Similar to sign-in and sign-up flows, Kinde also provides the `logout` method and `LogoutLink` component, which you can use to end the current session.
...
The SDK also provides `isAuthenticated` and `isLoading` state to prevent rendering errors.
...
The `has` function is a helper function that checks if a user has specific roles, permissions, feature flags, or billing entitlements. It returns a Promise that resolves to a boolean value indicating whether the user meets all the specified criteria.

```jsx
import { has } from '@kinde-oss/kinde-auth-react/utils';

// Basic usage - check multiple criteria at once
const userHasAccess = await has({
  roles: ['admin'],
...
**Return value:** `Promise<boolean>` - resolves to `true` if user meets all criteria, `false` otherwise
...
const AdminPanel = () => {
  ...
  return canAccess ? <div>Admin content</div> : <div>Access denied</div>;
};
```
...
const checkUserAccess = async () => {
  const canAccessAdminPanel = await has({
  ...
  const canUseDarkTheme = await has({
    featureFlags: [{ flag: 'theme', value: 'dark' }],
...
### Sign up and sign in users to organizations

To sign up a user to a particular organization, pass the `orgCode` from your Kinde account when the user is created. You can find the `orgCode` on the **Details** page of each organization in Kinde.
...
Following authentication, Kinde provides a JSON Web Token (JWT) to your application. In addition to standard claims, the token includes the `org_code` and the `permissions` for that organization (this is important because a user can belong to multiple organizations and have different permissions for each). Example of a returned token:
...
After a user signs in, your project or application receives a JWT token containing an array of permissions for that user. Configure your project to read permissions and unlock the respective functions.
...
To minimize the payload in the token, we use single-letter keys and values where possible. The single letters represent the following:
...
await getFlag('create_competition', {defaultValue: false});
/*{
...
When you refresh the browser using a front-end SDK, the authentication state is lost. There is no secure way to persist this in the front end.
...
2. For local development, use the escape hatch `useInsecureForRefreshToken`. Do **not** use this in production. We recommend using a custom domain instead. This stores only the refresh token in local storage and is used to silently re-authenticate.
...
It has been replaced by `callbacks > onSuccess`.
