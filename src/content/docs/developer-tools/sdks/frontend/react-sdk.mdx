...
Kinde provides a `useKindeAuth` hook with the `login` and `register` methods to start the flow, as well as `LoginLink` and `RegisterLink` components.
...
There are three callback events for handling authentication results and related SDK events.
...
- `onEvent` - Called when the SDK emits an auth-related event, such as a token refresh. It includes the event type, the related state, and the current context.
...
The `has` function is a helper function that checks if a user has specific roles, permissions, feature flags, or billing entitlements. It returns a Promise that resolves to a boolean indicating whether the user meets all the specified criteria.
...
**Return value:** `Promise<boolean>` - resolves to `true` if the user meets all criteria, otherwise `false`
...
We have a range of backend SDKs available to assist you in securing your back end application using this token, alternatively you can use any JWT validator and decoder to assist you here.
...
To create a new organization during registration, pass `isCreateOrg` to the login or register method, or to the relevant component.
...
await getClaim("givenName", "idToken");
...
```jsx
await getPermission("read:todos");
```
