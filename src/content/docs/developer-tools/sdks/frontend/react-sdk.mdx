...
If you are currently using v4, refer to this [migration information](#migration-from-v4-to-v5) to update to v5.
...
4. Import the `KindeProvider` component and wrap your application in it.
    ...
    Replace the `<your_kinde_client_id>` and `<your_kinde_domain>` placeholders in the code block below with the the values from the **Details > App keys** section.

Now that you have setup your Kinde React application, check out the rest of the documentation to configure your app.
...
```
With the hook, you can also pass additional parameters to the auth url.
