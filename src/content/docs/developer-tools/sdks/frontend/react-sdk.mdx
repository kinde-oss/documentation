...
```jsx
import { LoginLink, RegisterLink } from '@kinde-oss/kinde-auth-react';
...
- `onSuccess` - On Successful authentication, this includes the user authenticated along with the passed state
- `onError` - When an error occors during authentication, this includes the error along with the passed state

```jsx
<KindeProvider
    callbacks={
        {
          onSuccess: (user, state) => console.log("onSuccess", user, state),
...
```jsx
import { LogoutLink } from '@kinde-oss/kinde-auth-react';
...
`onRedirectCallback` has been removed in favour of a richer event system. 

This has been replaced by `callbacks > onSuccess`

### Accessing access token

`getToken` has been replaced by `getAccessToken`
