...
description: "Register APIs in Kinde, set audiences, import OpenAPI scopes for MCP, and manage app authorization and API keys"
sidebar:
  order: 1
relatedArticles:
...
featured: false
deprecated: false
ai_summary: "Guide to registering and managing custom APIs in Kinde so front-end and back-end apps can authenticate securely. Covers registering an API with a name and unique audience, authorizing applications, and including the audience in user tokens via SDKs such as React or in M2M tokens via the client credentials grant. Explains how Kinde validates the audience and returns the aud claim. On Kinde Plus and higher, you can upload OpenAPI 3.x or Swagger 2 files to generate scopes and MCP tool metadata, including upstream base URL from servers[0].url for MCP proxying. Also covers creating and scoping API keys, authorizing or revoking app access, and deleting APIs. Intended for developers and admins setting up API access in Kinde."
---
...
### OpenAPI specs and MCP connections

When you upload an OpenAPI spec, Kinde also stores MCP tool metadata for each operation (HTTP method, path, tool name, and description). The first server URL in the spec (`servers[0].url`) is stored as the upstream base URL used when Kinde proxies MCP tool calls to your API.

If you use [MCP connections for your APIs](/mcp-servers/add-mcp-connection/):

- Re-importing a spec updates tools on active connections that use that API.
- Scopes generated from the spec control which tools a user API key can access when connected via MCP.

Include a reachable `servers[0].url` in the spec so upstream tool calls have a valid base URL.
