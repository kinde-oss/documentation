...
.NET authentication requires an audience. If you don’t already have an audience defined, you will need to do this by [registering your API](https://docs.kinde.com/developer-tools/your-apis/register-manage-apis/). 
...
[Set up permissions](https://docs.kinde.com/manage-users/roles-and-permissions/user-permissions/#_top) in Kinde. 

### Via role claims

1. [Set up Roles](https://docs.kinde.com/manage-users/roles-and-permissions/user-permissions/) in Kinde.
2. Add roles to the access token via custom claims, see the [token customization](https://docs.kinde.com/build/tokens/token-customization/) procedure. 
...
4. Add the callback URL to your front end application’s [allowed callbacks](https://docs.kinde.com/get-started/connect/callback-urls/) so that the Swagger UI initiates auth and returns to Swagger UI after auth.
    
    For local development the callback will look like the following, though port may differ:
