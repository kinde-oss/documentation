...
7. (If you are using scopes) Add scopes to the token request. Enter scope values separated by a space, e.g. `read:calendar update:event delete:event`
8. Set the `audience` to `https://[yourdomain].kinde.com/api` (replacing [your domain]). To do this:

    a. Scroll down and expand the **Advanced** section. 
    
    b. Add `audience` as the **key** and paste the URL as the **value**.
    
    c. Set the **Send in** to `Request body`

9. In the **Authorization** section, select **Get New Access Token**. If it works, you should see a confirmation message.

   ![Access Token in Postman](https://imagedelivery.net/skPPZTHzSlcslvHjesZQcQ/b736bc91-3f30-4d48-62c6-37f131e88300/public)

10. Select **Use Token**. You should now have the access token.
