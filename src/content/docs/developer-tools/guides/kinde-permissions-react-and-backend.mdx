...
<Aside>
For production, extend this pattern with correct effect dependencies, auth/loading guards, error handling, and cleanup for in-flight async work.
</Aside>
...
export default function TodoHeader() {
  const [canCreateTodos, setCanCreateTodos] = useState(false);
  const [canManageUsers, setCanManageUsers] = useState(false);

  useEffect(() => {
    const checkAccess = async () => {
      const createTodosAccess = await has({ permissions: ["create:todos"] });
      const manageUsersAccess = await has({
        roles: ["admin"],
        permissions: ["manage:users"],
      });

      setCanCreateTodos(createTodosAccess);
...
It returns a Promise that resolves to `true` only when all requested conditions pass.

Examples:

- `await has({ permissions: ["create:todos"] })`
- `await has({ roles: ["admin"], permissions: ["manage:users"] })`

Use this for conditional UI rendering after resolving the async check, like showing a **New todo** button only when `create:todos` exists.
