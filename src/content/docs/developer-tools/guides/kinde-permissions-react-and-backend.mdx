...
Check the audience claim `aud` in your backend code to verify the token is intended for your API.

```typescript
// backend.ts
const { sub, aud, permissions = [] } = decodedToken;

if(aud.includes("your_intended_audience")) {
  // token is intended for your API
} else {
  // token is not intended for your API
}
```