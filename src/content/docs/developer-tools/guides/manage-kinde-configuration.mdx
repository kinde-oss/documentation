...
featured: false
deprecated: false
ai_summary: "This guide explains how to manage Kinde configuration as code across multiple environments (development, staging, and production) using a JavaScript seeding script and the Kinde Management API. Rather than manually recreating settings in each environment, you define your Kinde setup once in JSON config files — one per environment — and run a Node.js script to apply the configuration automatically. The guide covers setting up the seed script, writing helper functions for each resource type (applications, environment variables, feature flags, APIs and scopes, roles and permissions), creating per-environment JSON config files, and running the script with M2M app credentials. It also shows how to verify the results in the Kinde dashboard, and includes a FAQ covering current limitations such as the absence of cross-environment copying, least-privilege security practices, what resources to include under configuration as code, and how to create additional Kinde environments."
---
...
2. You will see that the script created the following:
