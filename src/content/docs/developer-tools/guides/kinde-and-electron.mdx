...
2. Add the following code to `index.html` file to create the UI:
    ...
    In production, register `win.webContents.setWindowOpenHandler` so external links (footer help center, docs CTA) open with `shell.openExternal` instead of navigating your `BrowserWindow`.
