...
description: Step-by-step guide to deploy your Next.js application with Kinde authentication to Cloudflare Workers using the OpenNext adapter, including wrangler.toml configuration, environment variable setup, callback URL configuration, and troubleshooting common deployment issues.
sidebar:
  order: 4
relatedArticles:
...
2. Update your `wrangler.toml` to include the following variables. You can get these from your Kinde application's **Quick start** page.
    ...
    <Aside type="warning" title="Important">
    When copying values from your `.env.local` file, make sure not to include the `KINDE_CLIENT_SECRET` value because the `wrangler.toml` file is only for storing public variables.
    </Aside>
