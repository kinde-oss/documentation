...
Kinde supports the Proof Key for Code Exchange (PKCE) extension. When using PKCE, include the `code_challenge` and `code_challenge_method` parameters in the authorization request. This flow is recommended for mobile apps and single-page applications (SPAs).
