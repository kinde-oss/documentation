...
| `email`, `sms` | A one-time code used as the *first* factor (passwordless sign-in) |
| `otp:email`, `otp:sms` | A one-time code used as a *second* factor |
...
`phr` is stronger: an enrolled passkey cannot be used by someone who only controls the account that provided the first factor, because the private key never leaves the authenticator. It is not a guarantee of independence, though - like most identity providers, Kinde allows a passkey to be enrolled from a session established by a single factor, so an attacker who compromises the first factor may be able to register one.

For payments and regulated workloads, assess factor independence, enrolment and recovery against your own configuration rather than relying on the `acr` value alone.
