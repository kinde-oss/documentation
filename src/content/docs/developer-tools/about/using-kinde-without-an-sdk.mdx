...
The implicit flow hands tokens straight to the browser, where they can be read from the address bar, browser history, or a `Referer` header, and it gives the authorization server no way to check that the app redeeming them is the app that started the flow. Requests that ask for it are rejected - see [`response_type`](#response_type) for the error you get back. The same applies to the hybrid flows, which also return a token in the redirect.
...
Must be `code`. Kinde does not support the implicit flow as it has shown to be unsecure.

Any other value - including `token`, `id_token`, `id_token token`, and the hybrid combinations `code id_token`, `code token` and `code id_token token` - is rejected:
...
The rejection is also recorded against your application, so you can see it in the errors list for that application in Kinde.
