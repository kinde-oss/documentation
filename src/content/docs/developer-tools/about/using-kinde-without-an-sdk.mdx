...
description: "Build Kinde auth without an SDK using OAuth 2.0 and OpenID Connect—code flow, PKCE, tokens, and route protection"
sidebar:
  order: 2
tableOfContents:
  maxHeadingLevel: 3
...
<Tabs syncKey="auth-flow">
  <TabItem label="Backend application">
    2. In your app, generate a random `state` value and a `nonce`:
    ...
    3. Send the user to Kinde's authorization endpoint with the `state` and `nonce` parameters included in the URL:
  ...
  <TabItem label="SPAs and mobile apps">
    2. In your app, generate a random `state` value, a `nonce`, a `code_verifier`, and derive a `code_challenge` from it:
        ...
        Store `state`, `nonce`, and `code_verifier` in `sessionStorage` (for SPAs) and secure device storage (Keychain/Keystore) for mobile apps— you will need them when the user returns to the callback URL.

    3. Send the user to Kinde's authorization endpoint with the `state`, `nonce`, and PKCE parameters included in the URL:
...
2. Decode the `id_token` with any standard [JWT library](https://www.jwt.io/libraries), or if you are using JavaScript, use the [Kinde JavaScript library](/build/tokens/decode-jwts/). Validate that the `nonce` claim matches the value you stored earlier to prevent replay attacks.
...
Must be `code`. Any other value is rejected — including `token`, `id_token` and `id_token token`, and the hybrid combinations `code id_token`, `code token` and `code id_token token`, which also return a token in the redirect. See [Does Kinde support the Implicit Flow?](#does-kinde-support-the-implicit-flow) for what you get back.
...
No. `response_type=code` is the only value Kinde accepts. Anything else is refused with an `unsupported_response_type` error before authentication starts:

```json
400 Bad Request
{
  "error": "unsupported_response_type",
  "error_description": "The authorization server does not support obtaining a token using this method. Kinde supports response_type 'code' only; 'token' is not supported."
}
```

The same applies to the hybrid flows — `code id_token`, `code token` and `code id_token token` — because they also return a token in the redirect, with the same exposure described in [Implicit flow (Not recommended)](#implicit-flow-not-recommended).

Each refusal is recorded against your application, so you can also find it in that application's errors list in Kinde.

To migrate an existing app, see [Implicit flow (Not recommended)](#implicit-flow-not-recommended).
