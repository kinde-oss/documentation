...
description: Learn how to customize Kinde tokens by adding custom claims, properties, and third-party integrations like Hasura for enhanced token functionality.
sidebar:
  order: 5
relatedArticles:
...
featured: false
deprecated: false
ai_summary: Guide for customizing Kinde tokens by adding custom claims, properties, and third-party integrations including step-by-step configuration for access, ID, and M2M tokens.
---
...
Token customization is a step toward enabling custom claims, but it is not the same thing. Standard JWT claims such as `sub` (subject) cannot be overridden. Kinde sets these and they are read-only.
