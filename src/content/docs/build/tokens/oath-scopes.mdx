...
Requests to `/oauth2/auth` that ask for the implicit flow, or for one of the hybrid flows that also returns a token in the redirect, are rejected with `unsupported_response_type`. Use `response_type=code` instead.
