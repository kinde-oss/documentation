...
Environment variables are key-value pairs configured outside your source code so that each value can change depending on the Environment (link to env). Common use cases are for API keys, and URLs or IDs which change per environment, as they can be more easily updated wherever the key is referenced. You can store as many environment variables in Kinde as you want. 
