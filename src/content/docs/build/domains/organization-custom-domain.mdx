...
## Step 4: Update application callbacks

1. In Kinde, go to **Settings > Applications**.
2. In each relevant application, add the custom domain callback URL (`account.mydomain.com/login/callback`) to the **Allowed callback URL** list.
3. Select **Save**.

Remember to also update your application's codebase to reference the custom domain and callback.

## Step 5: Update provider social auth apps
