...
description: "Learn Kinde billing terms—fixed charges, mixed-interval pricing, Pay annually, Next invoice, trials, and token claims"
featured: false
deprecated: false
topics:
  - billing
...
- **Next invoice** – In the self-serve portal and admin billing views, the upcoming charge preview: licensed amounts due on the next charge date plus metered usage recorded so far. Shown as an amount and date without a `/ month` or `/ year` label. May include a caption that the total can still change with usage before the invoice is issued.
- **Pay annually** – The toggle on plan selection (sign-up and self-serve portal) that switches displayed prices and CTAs from the standard interval to annual (and mixed) amounts. Defaults to off. Appears only when at least one displayed plan offers annual pricing.
