...
description: "See how Kinde billing works with Stripe—B2B and B2C, mixed-interval charges, and Next invoice in portal and admin"
featured: false
deprecated: false
topics:
  - billing
...
- Billing cycles depend on the interval you set for each fixed charge (daily, weekly, monthly, or yearly), based on the customer’s original sign-up date.
- Fixed charges (for example subscription fees) are billed in advance at the interval you choose; customers pay at the start of their billing period. You can also offer one fixed charge per plan as annual at a custom yearly price.
- A single plan can combine intervals—for example an annual base fee plus a monthly fixed add-on (`$100 / year + $3 / month`). Each charge renews on its own schedule.
