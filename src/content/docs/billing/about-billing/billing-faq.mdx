...
You can enable **Collect tax automatically** in **Settings > Environment > Billing** (off by default). When enabled, Kinde turns on Stripe Tax for new subscriptions. Individual plans can inherit that setting via **Use account setting**, or override it with **Collect** or **Don’t collect**.

Stripe Tax calculates tax amounts; Stripe Billing presents them on invoices. Configure tax registrations and related Stripe Tax options in the Stripe Dashboard for the regions where you sell. Until a customer has a billing country (for example on a trial before a card is added), tax may use your Stripe account’s registered address.
