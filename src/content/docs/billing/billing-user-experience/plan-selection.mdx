...
description: "Build Kinde pricing tables with Pay annually, plan highlights, translations, and live signup or self-serve display"
featured: false
deprecated: false
topics:
  - billing
...
keywords:
  ...
  - live pricing
updated: 2026-08-22
ai_summary: "Guide to building and publishing pricing tables in Kinde so customers can select plans during signup or self-serve upgrades. Covers generating a table from a plan group or creating a blank table, the four-plan limit, editing content without versioning, and that plan prices are inherited from the plan. Explains How customers choose annual pricing: Pay annually appears when a displayed plan offers annual pricing, defaults to off, applies at registration and portal plan changes, shows mixed yearly plus monthly amounts when relevant, and cannot switch the current plan between monthly and annual. Describes amount-due display for mixed, pure annual, and monthly checkout. Also covers adding and removing plans, ordering, feature lists, highlight labels, translations, making a table live, showing it by default in register, hiding it from signup while keeping it in the portal, and adding plan selection to registration URLs. Intended for developers, product managers, and UX designers."
---
...
## How customers choose annual pricing

If any plan on the pricing table offers annual pricing (via **Offer as annual** on a fixed charge), customers see a **Pay annually** toggle on the plan selection screen. See [Offer annual pricing](/billing/manage-plans/create-plans/#offer-annual-pricing) for how to set this up on a plan.

This applies to:

- Sign-up when the pricing table is shown in the registration flow
- Self-serve portal plan changes (upgrade / downgrade)

### Default and visibility

- **Pay annually** starts **off**. Customers see monthly (or standard-interval) prices and CTAs until they turn it on.
- The toggle appears only when at least one **displayed** plan card offers annual pricing.
- A pricing table shows up to **4** plans. If only a plan that is not on the table offers annual pricing, **Pay annually** does not appear.

Hovering the toggle shows a localized tooltip explaining that selecting it shows annual prices. There is no helper text under the switch.

### What customers see when Pay annually is on

- Plan cards switch to annual amounts (and mixed amounts when the plan has both yearly and monthly fixed charges)—for example `$100 AUD / year + $3 AUD / month`.
- Choosing a plan continues with annual prices selected for that subscription.

<Aside>

**Pay annually** only affects other plans the customer can move to. Customers cannot use this toggle to switch the **same** plan between monthly and annual billing. The toggle always starts off, including when the customer is already on a mixed annual + monthly agreement.

</Aside>

### Amount due at payment (sign-up)

On collect payment details:

- **Mixed annual offer:** the headline is the amount due now (for example `$103 AUD`) with a recurring breakdown underneath (for example `$100 AUD / year + $3 AUD / month`). The total is not labeled `/ year`.
- **Pure annual:** the total still shows with `/ year`.
- **Monthly:** unchanged (`/ month`).
