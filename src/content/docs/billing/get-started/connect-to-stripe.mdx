...
description: "Connect Kinde to Stripe, finish onboarding, enable automatic tax, and set cancellation and plan-change billing policies"
featured: false
deprecated: false
topics:
  - billing
...
keywords:
  ...
  - connection status
  - test Stripe account
ai_summary: "Step-by-step guide to connecting Kinde billing to Stripe and configuring related environment settings. Explains that Kinde creates a new Stripe account for billing and that existing Stripe accounts cannot be connected directly, though business details can be copied during onboarding due to a Stripe third-party integration limitation. Covers testing in a non-production environment with a Stripe test account, starting the connection from Settings Environment Billing, completing Stripe onboarding via Update Stripe information, and interpreting connection statuses Connecting, In progress, and Connected. Describes enabling Collect tax automatically so Kinde turns on Stripe Tax for new subscriptions, with per-plan overrides still available, and notes that tax registrations must be completed in the Stripe Dashboard. Also covers setting billing policies for cancellations and plan changes, including how unpaid metered usage and unused paid subscription days are handled. Intended for developers, product managers, and business owners setting up Kinde billing payments."
updated: 2026-08-10
---
...
After Stripe is connected, you can turn on automatic tax for new subscriptions.

1. Go to **Settings > Environment > Billing**.
2. In the **Settings** section (alongside **Default currency**), turn on **Collect tax automatically**.
3. Select **Save**.

    ![Collect tax automatically setting in Kinde](https://imagedelivery.net/skPPZTHzSlcslvHjesZQcQ/52ea5044-ab13-425b-f1dd-a09c0eac7600/socialsharingimage)

This setting is off by default. When it is on, Kinde enables Stripe Tax on new subscriptions unless a plan overrides it with **Collect** or **Don’t collect** in plan settings.

You still need to complete Stripe Tax setup in the Stripe Dashboard (for example tax registrations for the regions where you sell). Until a customer has a billing country—such as during a trial before a card is added—tax may temporarily use your Stripe account’s registered address.
