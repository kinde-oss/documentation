...
app_context:
  ...
  - m: settings
    s: business_details
description: Comprehensive guide to exporting Kinde data including secure password export processes, encryption/decryption, and data format specifications.
topics:
  - manage-your-account
  - your-data
sdk: []
...
featured: false
deprecated: false
ai_summary: Comprehensive guide to exporting Kinde data including secure password export processes, encryption/decryption, and data format specifications.
---
...
Data is exported in NDJSON format. Your export includes three data files:

| File | Contents |
|------|----------|
| `organizations.ndjson` | All your organizations |
| `users.ndjson` | All registered users (with optional password hashes) |
| `access_requests.ndjson` | People who've requested access but aren't users yet |

<Aside>

Access requests are people who have submitted a request through your [Request Access page](/design/pages/set-up-the-request-access-page/), such as early adopters on a waitlist or people requesting access to a beta or invite-only product. They're included automatically—no additional setup required. Note that access requests who have been converted to full users will appear in both the `users.ndjson` and `access_requests.ndjson` files.
...
Here's an example of `access_requests.ndjson`.
...
```json
{
  ...
  "properties": {
    ...
    "first_name": {
      "type": ["string", "null"],
      "description": "First name provided by the requester"
    },
    "last_name": {
      "type": ["string", "null"],
      "description": "Last name provided by the requester"
    },
    "email": {
      "type": "string",
      "format": "email",
    ...
    "business_code": {
      "type": "string",
      "description": "Your Kinde business identifier"
    },
    "organizations": {
      "type": "array",
      "items": {
      ...
      "description": "Organizations the requester is associated with, if any"
    },
    "created_on": {
      "type": "string",
      "format": "date-time",
      "description": "Timestamp when the access request was submitted"
    }
  },
  "required": ["id", "email", "business_code", "created_on"],
  "additionalProperties": false
}
```
