...
With automation things can go wrong. That’s why we provide both sync logs and runtime logs to help you investigate any issues that arise. 

## Workflow sync log

When your code hits Kinde from a fresh commit in your repo, we check to make sure we’re able to run it. If these checks fail, this will show up in the logs for the deployment. 

1. Go to **Workflows > Sync logs**. This shows a page list of all the sync events in chronological order.
