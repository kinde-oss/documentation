...
In this tutorial, we’ll focus on how to deny cancelations for Enterprise plans, ensuring that customers who have signed legal agreements can only cancel by contacting support - while still allowing self-serve cancelation for other plans.
...
7. Run the following git commands in your terminal to push the changes to your GitHub repo:
    ...
    ```bash
    git add .
    git commit -m "add new deny plan cancelation workflow"
    git push
    ```
...
2. Open your Kinde project and go through the signup process. You will see the plan selection page. 
...
By adding a workflow to handle cancelation requests, you can create clear rules for cancelling plans directly in the self-serve portal. 
