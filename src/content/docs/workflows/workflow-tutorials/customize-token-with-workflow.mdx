...
Define the custom claim property you want to add to the access token, then update it with your custom value:
...
To remove claims from the access token, update the custom claim property to an empty array or object:
...
Define the custom claim property you want to add to the ID token, then update it with your custom value:
...
2. Import and use the `M2MTokenGeneration` trigger (alias for [`m2m:token_generation`](/workflows/example-workflows/m2m-token-generation-workflow/)):

    ```typescript
    trigger: WorkflowTrigger.M2MTokenGeneration,
    ```

2. Define the custom claim property you want to add to the M2M token, then update it with your custom value:
