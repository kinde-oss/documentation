...
This trigger is fired after a new password is entered in either:
...
For gradual migrations to Kinde where several apps are in play - for example a mobile application and a web based application, it might be the case that the web migration is carried out first and mobile app later. If users have access to both applications then password resets on the web application would not be persisted to the legacy mobile app password store. 
