...
This trigger is fired when user tokens are generated (ID and access).
...
To ensure the integrity of Kinde issued tokens there are some claims which cannot be modified: https://github.com/kinde-oss/infrastructure/blob/main/lib/prohibitedClaims.ts
...
Kinde automatically populates the access token with `permissions` and `feature_flags` . If your product leverages these features heavily the access token can quickly grow in size and you may prefer to instead use the management API to get these data points and strip them from the token
