...
tableOfContents:
  maxHeadingLevel: 3
description: "Add Kinde auth to your existing codebase—SDK setup, keys and callbacks, test environments, Management API, and deploy to production."
relatedArticles:
...
featured: false
deprecated: false
ai_summary: "Guide for developers adding Kinde authentication to an existing application codebase after completing Kinde onboarding. The quickstart walks through creating or reusing a Kinde application, choosing a starter kit or manual SDK installation, setting callback and redirect URLs, copying domain, client ID, and client secret app keys into a .env file, and testing user registration locally. The build and deploy section covers creating non-production test environments, registering back-end APIs so they appear in access token audience claims, setting up a machine-to-machine application to call the Kinde Management API with client credentials, and deploying to production with updated callback URLs and environment variables, with links to Vercel, Netlify, and Cloudflare Workers guides. Advanced topics link to Workflows, webhooks, cross-environment configuration, and end-to-end testing. Prerequisites include a Kinde account with Admin or Engineer permission and familiarity with your chosen programming language and framework."
---
...
- A [Kinde](https://kinde.com) account with **Admin** or **Engineer** permission
...
2. Enter a name for the application (e.g., "My Next.js App"), select the type of application, and select **Save**.
    ...
    <img src="https://imagedelivery.net/skPPZTHzSlcslvHjesZQcQ/afb04c69-e3b0-4afe-385d-00e05346e600/socialsharingimage" alt="add new application in kinde" width="60%" height="auto" fetchpriority="low" loading="lazy" decoding="async" />
...
3. Select the application language/framework (e.g., Next.js) and select **Save**.
    ...
    ![list of backend sdks in kinde](https://imagedelivery.net/skPPZTHzSlcslvHjesZQcQ/b793e938-0f4f-4209-7f81-e14309925b00/socialsharingimage)
...
2. Copy the following:
    ...
    - **Domain** (`yourbrandname.kinde.com` or your [custom domain](/build/domains/pointing-your-domain/))
    - **Client ID**
...
1. Run your app locally and select the **Sign up** button. You'll be redirected to the Kinde login page.
    ...
    <img src="https://imagedelivery.net/skPPZTHzSlcslvHjesZQcQ/e13f9836-cb8a-4f06-5556-69d668a5a200/socialsharingimage" alt="kinde hosted login page" width="60%" height="auto" fetchpriority="low" loading="lazy" decoding="async" />
...
1. Go to your Kinde dashboard, and select the arrow next to the production environment dropdown (top-left of the screen). A pop-up appears.
    ...
    ![kinde all environments dropdown](https://imagedelivery.net/skPPZTHzSlcslvHjesZQcQ/0969d059-f555-42af-3423-899b35ec1c00/socialsharingimage)
...
6. Use the `client_credentials` grant type to get a token and make authorized requests to the Kinde Management API. See: [Call the Management API](/developer-tools/kinde-api/access-token-for-api/).
