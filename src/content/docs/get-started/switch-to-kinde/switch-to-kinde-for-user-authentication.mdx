...
Kinde can import files up to 50MB.

### Migrating from Auth0

When you export user data from Auth0, export all the default fields.

Note that if you want to bring passwords as well, you will need to request a special JSON file through the Auth0 support desk. This can take up to 2 weeks.

### Migrating from Azure (now MS Entra ID)

We recommend you set up the [Entra ID connection in Kinde](/authenticate/enterprise-connections/azure/) before you import your users.

When you import user details from Azure they must include an Azure ID/Entra ID so Kinde can match them to the connection you set up.
