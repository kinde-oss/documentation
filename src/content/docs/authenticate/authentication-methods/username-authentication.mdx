...
When a user signs in, they enter their username and proceed with a [password](/authenticate/authentication-methods/password-authentication/).
...
Once an email is verified, we add this email identity for the user. If in future, the authentication pattern changes, the user can opt to sign in with their email and receive OTPs. An email is also required for [password resets](/manage-users/access-control/reset-user-password/).
