...
description: "Let users sign in with a unique username plus password or OTP, using a verified email for codes and resets"
featured: false
deprecated: false
topics:
  - authenticate
...
keywords:
  ...
  - sign in with a code
ai_summary: "Guide to username-based authentication in Kinde, where a username is the sign-in identity after a one-time email verification on sign-up. Covers how usernames can be added manually, via API, by the user, or on import, uniqueness and case-insensitivity rules, character limits, and optional custom validation with Workflows. Explains the sign-up flow requiring email plus username, the sign-in flow with password or passwordless OTP, and the option to request an emailed one-time passcode from the password page sent to the verified profile email rather than the username, with a link to the dedicated guide for using password and passwordless auth at the same time. Notes why email is required for identity verification, password resets, and code sign-in. Describes enabling Username password or username plus code connections, and that users share a single password across email and username identities. Intended for developers and product managers."
updated: 2026-08-22
---
...
If you use username + password, you can also let users request a one-time passcode instead of entering their password. Kinde sends the code to the verified email address on their profile, not to their username. See [Use password and passwordless auth at the same time](/authenticate/authentication-methods/use-password-and-passwordless-auth/).
...
Once an email is verified, we add this email identity for the user. If the auth method is passwordless, this is where we send the OTPs. An email is also required for [password resets](/manage-users/access-control/reset-user-password/) and for signing in with a code from the password page.
