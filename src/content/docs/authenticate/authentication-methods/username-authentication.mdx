...
If you use username + password, you can also let users request a one-time passcode instead of entering their password. Kinde sends the code to the verified email address on their profile, not to their username. See [Let users sign in with a code instead of a password](/authenticate/authentication-methods/password-authentication/#let-users-sign-in-with-a-code-instead-of-a-password).
