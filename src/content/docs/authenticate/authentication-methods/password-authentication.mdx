...
description: "Enable hashed password sign-in with reset, lockout, temporary passwords, and an optional emailed one-time code"
featured: false
deprecated: false
topics:
  - authenticate
...
keywords:
  ...
  - email + password
  - account lockout
ai_summary: "Guide to password authentication in Kinde. Explains when passwords are appropriate and recommends stronger options such as one-time passcodes and multi-factor authentication. Covers verified contact identity requirements for resets, hash-encrypted password storage that is never visible to admins or Kinde, and strength rules including an eight-character minimum, blocking of common passwords, lockout after failed attempts, and no complexity requirements. Describes custom password rules via a Kinde workflow. Covers reset options: user-initiated forgot password, admin or API-triggered reset, and issuing a temporary password that must be communicated outside Kinde. Notes that users can request an emailed one-time passcode from the password page and links to the dedicated guide for using password and passwordless auth at the same time. Includes steps to enable Email plus password or Username password authentication across apps or for a single application. Intended for developers and product managers."
updated: 2026-09-05
---
...
If you want to add custom password requirements such as use of special characters, numbers, and minimum length, use a custom Kinde workflow. See [Password validation check workflow](/workflows/workflow-tutorials/custom-password-validation-check/) for more information.
...
You can offer password and passwordless sign-in in the same flow. When the option is on, users see **Sign in with a code** on the password page and can request an emailed one-time passcode without resetting their password.

See [Use password and passwordless auth at the same time](/authenticate/authentication-methods/use-password-and-passwordless-auth/).
