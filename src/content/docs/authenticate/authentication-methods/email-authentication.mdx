...
When a user signs up with email and they have a [Gravatar](https://docs.gravatar.com/) attached to their email account, the image URL is passed through when they authenticate. If a user does not have a Gravatar image, Kinde uses a fallback Gravatar URL for a transparent image. If you don't want to use the fallback Gravatar, you can switch this off. 
