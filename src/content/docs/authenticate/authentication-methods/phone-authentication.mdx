...
You can allow users to use SMS as their primary or secondary means of authentication. This is a passwordless method, where the user is sent a code via an SMS to verify them on sign in. SMS can also be included as secondary factor is you have [multi-factor identification](/authenticate/multi-factor-auth/about-multi-factor-authentication/) set up. 
