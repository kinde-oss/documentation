...
No. WhatsApp authentication is available on every Kinde plan, including Free. Kinde does not cap the number of WhatsApp messages you can send and does not charge for WhatsApp delivery. WhatsApp is a bring-your-own-provider integration, so Meta charges you separately for each message sent — see [WhatsApp pricing](https://developers.facebook.com/docs/whatsapp/pricing) for details.

The [Kinde default SMS](/authenticate/authentication-methods/phone-authentication/#kinde-default-sms) allowance applies to SMS only. If WhatsApp delivery fails and Kinde falls back to SMS, those fallback messages are sent through your SMS configuration, so they count toward your Kinde SMS allowance when you use Kinde default SMS.
