...
keywords:
  ...
  - username + code
  - sign in with a code
ai_summary: "Guide to passwordless authentication in Kinde using one-time passcodes (OTP) instead of user-managed passwords. Explains why Kinde uses OTPs rather than magic links, that codes can be sent by email or phone, that they expire after two hours, and that they are also used in multi-factor authentication. Covers setup for Email connections Code, Phone connections Code, and Username connections Code, including Twilio and paid-plan requirements for phone. Describes offering a Sign in with a code button to password users on Email plus password or Username connections, and points to the dedicated guide for using password and passwordless auth at the same time. Includes troubleshooting if a user does not receive a code, including verified-email checks for the password-page flow, and links to attack protection settings. Intended for developers and product managers."
updated: 2026-08-22
---
...
This is set per connection, in the password connection's configuration. See [Use password and passwordless auth at the same time](/authenticate/authentication-methods/use-password-and-passwordless-auth/).
