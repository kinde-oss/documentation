...
If a user signs up with a trusted enterprise connection that provides a verified email, and an email identity does not exist for the user, we create an email identity for them, which they can use to sign in with (if an email authentication method is enabled for your application).
...
Currently Google is the only provider we treat as trusted by default. But you can choose to trust any untrusted provider when you configure them in Kinde (See below). You can also choose not to trust Google if you want.
...
2. If LinkedIn is an untrusted provider, we only create a social identity.
...
![Kinde switch for trusting an identity provider](https://imagedelivery.net/skPPZTHzSlcslvHjesZQcQ/ff923113-34d0-4b8d-e969-db08e9a22700/public)
