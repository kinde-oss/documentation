...
sidebar:
  order: 3
relatedArticles:
  - 45afd09a-2e81-445b-8a57-263af57db796
...
This topic is a conceptual overview of how identity works in Kinde. There are three parts to a user’s identity in Kinde:

- Kinde profile - first name, last name, and picture
- Contact information fields - A mix of emails, phone numbers, and addresses for contact purposes.
- Identity details - information a user can sign in with. This is explained in more detail below.

## Identity

When we use the term ‘identity’, we’re referring to a unique identifier that allows a user to sign in and be verified. An email, phone number, etc.
...
### Profile / contact identities

This refers to an email, phone number, or username. A profile and contact identity is created for a user when they first sign up with an email address, phone number, or username. If it’s the first identity to be added, it will be marked as primary.
You can [manually add these user identities](/manage-users/access-control/add-manage-user-identities/.
...
An enterprise identity is created when a user signs up via an enterprise connection, or when they sign in via an enterprise connection which supplies a verified email that matches an existing email identity.

If a user signs up with an enterprise connection that provides a verified email, and an email identity does not exist for the user, we create an email identity for them, which they can use to sign in with (if an email authentication method is enabled for your application).

## Trusted providers

A trusted provider is one that guarantees the email they issue is verified and also does not allow the user to change their primary email address. 

Currently Google is the only provider on this list.

**Google example**

1. When a user signs up, the Kinde profile first name, last name and picture are populated from Google.
2. Kinde creates a primary contact email using the address supplied by Google.
3. Because Google is a trusted provider, we create a social identity and an email identity.
4. The user can sign in with either Google or their email (if this auth method is allowed).

## Untrusted providers

This isn’t to say that other providers are not trustworthy, this purely relates to email addresses and the guarantee that it will not change.

**LinkedIn example**

1. When a user signs up, the Kinde profile first name, last name and picture are populated from LinkedIn.
2. Kinde creates a primary contact email using the address supplied by LinkedIn.
3. Because LinkedIn is an untrusted provider, we only create a social identity.
4. At this stage the user can only sign in with their LinkedIn account.
...
3. If the same user returns and tries to sign in with their Google account. Kinde recognises that a user exists with the email address provided by Google, so we add a Google identity to the user because they are a trusted provider.
4. The Kinde profile first name, last name and picture are updated by Google (if profile syncing is turned on).
...
This is on by default, but you can turn it off if you wish to keep a static profile. [About profile sync](https://docs.kinde.com/manage-users/about/#user-profiles-are-synced-to-kinde).
...
You can update identities by adding a new identity and deleting the old one. See [Add and manage user identities](authenticate/about-auth/add-manage-user-identities/), or [via the Kinde API](https://kinde.com/api/docs/#kinde-management-api).
