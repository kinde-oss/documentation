...
**Support for sessions across domains and subdomains**
...
Once a user is signed in to one domain or subdomain, they can switch to another and remain authenticated. To do this, initiate the flow with `prompt=none` as part of the auth URL. This checks for a session on Kinde invisibly. If it exists the user experiences a seamless session between sites, and if a session doesn’t exist, they are prompted to reauthenticate.
