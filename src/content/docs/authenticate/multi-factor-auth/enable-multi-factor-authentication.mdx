...
5. Use the **Enable recovery codes** toggle to choose whether users see recovery codes when setting up MFA. This is **enabled** by default for backwards compatibility. If you turn it off, users will skip the recovery code generation and display step when setting up their Authenticator app or other MFA methods.
