...
Most identity providers accept longer values, but some enforce the limit and will truncate or reject the value. If your IdP rejects the RelayState URL, or the browser lands somewhere unexpected after the assertion, keep the host and path of your login route as short as possible. If it still doesn't fit, contact Kinde support.
