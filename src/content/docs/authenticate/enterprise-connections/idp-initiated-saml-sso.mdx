...
10. **Sign-in button**: Controls when the SSO button for this connection appears on the sign-in screen.
    - **Auto (default):** The button is shown only if the connection has no home realm domains. If home realm domains are set, the button is hidden and users are routed by email domain detection.
    - **Show:** The button is always visible on the sign-in screen, even if home realm domains are set.
    - **Hide:** The button is never shown. This does not affect IdP-initiated sign-in, which starts at the IdP and does not use the button.
