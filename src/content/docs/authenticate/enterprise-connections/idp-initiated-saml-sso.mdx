...
RelayState is how the IdP tells Kinde where to send the browser after the assertion is validated. For this flow, RelayState is required, and it must be a **full, absolute URL** starting with `https://`, or with `http://localhost` when you are testing locally.

A bare query string such as `client_id=...&connection_id=...` is not accepted. Neither is a relative path such as `/login?client_id=...` — Kinde does not resolve it against your Kinde domain. Both fail with error 9055.
...
<Aside type="warning" title="Keep the RelayState URL short">

The SAML 2.0 specification limits RelayState to 80 bytes. The sample URLs above already exceed that with placeholder values, and real `client_id`, `connection_id`, and `org_code` values make them longer again.

Kinde does not enforce this limit. It won't reject or truncate a longer RelayState value, and the RelayState Kinde generates for SP-initiated flows is well inside the limit. Any constraint you hit comes from your identity provider: most accept longer values, but some enforce the spec limit and will truncate or reject.

If your IdP rejects the RelayState URL, or the browser lands at a truncated URL, keep the host and path of your login route as short as possible. You can't drop `client_id` or `connection_id` to save space — Kinde requires both.
...
| --- | --- | --- |
| `client_id` | Identifies the Kinde application the user is signing in to | Required. Must use this exact query parameter name |
| `connection_id` | Identifies the enterprise connection to use | Required. Must use this exact query parameter name |
...
If either `client_id` or `connection_id` is missing from the RelayState URL, Kinde does not recognize the request as IdP-initiated and sign-in fails with error 9055. Include both, even though Kinde does not use `client_id` to look up your **Application login URI**.
...
| "Signature verification failed", or error 9055 on an otherwise correct setup | Kinde could not validate the SAML assertion — signature, issuer, audience, recipient, or validity window | Verify your IdP's signing certificate, Entity ID, and ACS URL match the connection in Kinde |
...
| Nothing happens after selecting the app tile, or you land on the Kinde login screen. Error 9055 | RelayState is only `client_id=...&connection_id=...`, is a relative path, or is not set in the IdP | RelayState must be a full URL including the scheme and host. Confirm it is saved in the IdP (Start URL, Default Relay State, or equivalent) |
...
| Nothing happens after selecting the app tile, and **Accept requests** is off. Error 9055 | IdP-initiated SSO is disabled on the Kinde connection | Switch **Accept requests** on (Step 2) |
...
| The IdP rejects the RelayState value, or the browser lands at a truncated URL | The RelayState URL exceeds the 80-byte limit in the SAML 2.0 specification and your IdP enforces it | Shorten the host and path of your login route. If it still doesn't fit, contact Kinde support |
| "Invalid client", or the connection isn't recognized. Error 6722 | The client ID and connection ID are from different Kinde environments, or the connection ID in RelayState doesn't match a SAML connection | Confirm both values come from the same Kinde environment and the connection ID is copied from the connection you configured |
