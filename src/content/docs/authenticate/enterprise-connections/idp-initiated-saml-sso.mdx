...
3. **IdP metadata URL**: You will add it after finish setting up your Identity Provider
...
1. **ACS URL / Reply URL**: `https://yourdomain.kinde.com/login/saml/callback`
...
5. **Enable IdP-initiated SSO**: In your IdP's application settings, enable the option to allow IdP-initiated sign-on (the exact setting name varies by provider)
    ...
    Once finished, you will receive the metadate URL and certificate information to add to your Kinde connection.  
...
2. Scroll to the IdP metadata URL field and paste the Metadata URL you copied from the last step.
...
7. Select Save. You can now use the IdP for the selected applications.
