...
Single Logout (SLO) is a SAML feature that allows users to be signed out of both the Service Provider (Kinde) and the Identity Provider (IdP) in a single action. This provides a complete logout experience and is considered a security best practice.

### SLO Configuration

When configuring single logout:

1. **Single Logout URL**: This is the endpoint where your IdP will send logout requests. Kinde provides this URL in the connection configuration.
2. **Logout Response URL**: Some IdPs require a specific response URL for logout confirmations.
3. **Binding Method**: SLO typically uses HTTP Redirect or HTTP POST binding, similar to authentication requests.

### Benefits of SLO

- **Complete Session Termination**: Users are signed out of all related sessions
- **Security**: Prevents unauthorized access from shared or public computers
- **User Experience**: Provides a clear, complete logout process
- **Compliance**: Meets security requirements for many enterprise environments

### IdP-Specific Considerations

Different identity providers may have varying requirements for SLO configuration:
- Some require specific URL formats
- Others may need additional parameters or headers
- Check your IdP's documentation for specific SLO requirements

<Aside type="warning">

Not all identity providers support single logout. If your IdP doesn't support SLO, users will only be signed out of Kinde, not the IdP session.

</Aside>

### Logout Experience Configuration
