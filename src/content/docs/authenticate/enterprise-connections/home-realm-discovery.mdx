...
Setting home realm domains on a connection hides its SSO sign-in button by default, so users are routed silently by email domain. You can override this per connection with the **Sign-in button** option:

- **Auto** (default) — hides the button when home realm domains are set.
- **Show** — keeps the button visible so users can choose to click through to the IdP.
- **Hide** — hides the button even when no home realm domains are set.

**Hide** does not switch off home realm discovery. Users with a matching email domain are still routed to the connection's IdP and signed in.

Learn more about [showing or hiding the SSO sign-in button](/authenticate/enterprise-connections/about-enterprise-connections/#show-or-hide-the-sso-sign-in-button-on-the-auth-page).
