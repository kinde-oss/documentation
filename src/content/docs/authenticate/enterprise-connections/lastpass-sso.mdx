...
15. Copy the **Single Sign-on URL** and the **Identity provider entity ID** and paste where you can access them again.
16. Make sure the **Name ID format** is `emailAddress` and the **Name ID** is `Email address'.
17. Enter any custom attributes required.

    ![Custom attributes in LastPass](https://imagedelivery.net/skPPZTHzSlcslvHjesZQcQ/cd749576-b884-42e9-d7cb-caf6bca32400/public)

18. Select **View key** under the **Key field**.
19. Copy the key. You will need this to finishe setting up the Kinde connection.
20. Make sure the **Service is enabled** option is selected.
21. Select **Save**.
