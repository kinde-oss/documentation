...
![option for JIT in the enterprise connection screen](https://prod-files-secure.s3.us-west-2.amazonaws.com/6108d6df-5299-4335-974c-e148a51dfa48/ebc489c0-b718-493d-b5e8-bd49a07b8f01/Untitled.png)
...
- The user already exists in Kinde and you now want them to connect via SSO.
...
You can add users to Kinde [via import](/manage-users/add-and-edit/import-users-in-bulk/) or [via API](https://kinde.com/api/docs/?http#create-user/). 
...
**Users who sign on through multiple enterprise connections.** It’s possible that you manage users who can sign in via multiple enterprise connections. In these cases, the user must have a separate profile for each enterprise connection.
...
Send an UPDATE request to the [create identity](https://kinde.com/api/docs/?http#create-identity/) endpoint via the Kinde API: 

`POST /api/v1/users/{user_id}/identities`

```jsx
need code snippet example
```
...
![Screen shot of trust emails switch](https://prod-files-secure.s3.us-west-2.amazonaws.com/6108d6df-5299-4335-974c-e148a51dfa48/6c66b597-2c1c-49e3-9bc3-24b0e6526a1e/Untitled.png)
...
If a user goes to sign in and encounters the ’We can’t find your account’ message, it could be for two reasons:

1. The **Allow self-sign-up** function is disabled in the environment settings. See **Settings > Policies**. We are fixing the issue so that JIT provisioning overrides the setting, but you can switch it on to fix as well.
2. Self-joining for the organization is switched off. This is the right behaviour if you don’t want users without the org_id to join the org, but the message is confusing. Switch this on via  **Organization > Policies**.
