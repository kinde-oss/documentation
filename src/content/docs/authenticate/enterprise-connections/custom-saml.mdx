...
Before you set up SAML, you can [import users in bulk](/manage-users/add-and-edit/import-users-in-bulk/), add them via [API](https://kinde.com/api/docs/#create-user), or [manually](/manage-users/add-and-edit/add-and-edit-users/) in Kinde. Alternatively, you can also take advantage of just-in-time (JIT) provisioning (Step 13 below) when you set up the connection.
