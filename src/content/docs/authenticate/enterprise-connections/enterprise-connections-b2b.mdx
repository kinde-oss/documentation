...
The easiest way to restrict an enterprise connection to an organization, is to add the connection to the organization and not create it as a shared connection (at the environment level). To do this, follow the relevant procedure for adding a connection in the relvant topic.

## Restrict org access for a shared enterprise connection
...
2. Deselect all auth methods except the shared enterprise connection you want.
3. Select **Save**.
