...
This guide is for when moving an existing user with an email identity to using an Enterprise Connection.

</aside>

### What you need?

- An existing [Enterprise Connection](/authenticate/enterprise-connections/) configured in Kinde (e.g., SAML or Entra ID)
- Users with email addresses that match exactly their Enterprise Identity (capitalization and structure must match)
- Access to configure the Enterprise Connection settings in Kinde
- A test application environment (recommended before enabling in production)
- Access to your IdP logs (e.g., Entra ID sign-in logs) for verification
...
11. Copy the **ACS URL**. You'll need to add this back to Entra ID as the reply URL (update step 8 above if you haven't done this yet).
...
4. **Test email/password login (optional):**
...
1. **Disable the Enterprise Connection** in Kinde to prevent further issues.
...
3. **Revert the test user** back to email/password identity if needed.
4. **Fix any configuration issues** and repeat the test.
...
1. Select several test users from different departments or teams.
2. Test across different browser configurations (Chrome, Firefox, Safari, Edge).
3. Have each test user complete the SSO login flow.
4. Monitor both Kinde and Entra ID logs for any errors or mapping problems.
5. Verify each user's profile, roles, and permissions are preserved correctly.
6. Document any issues encountered and resolve them before proceeding to full migration.
...
## Step 5: Post-migration checks and housekeeping
