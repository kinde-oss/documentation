...
description: "Set unique auth methods per organization with shared or enterprise connections, including connection-level code sign-in"
featured: false
deprecated: false
topics:
  - authenticate
...
ai_summary: "Guide to setting custom authentication methods per organization in Kinde for B2B customers. Explains environment-level shared connections versus organization-level enterprise connections, and that some options such as Show Sign in with a code on password page belong to the connection itself, so organizations sharing a password connection share that setting. Covers adding existing shared connections to an organization, that organization auth fully overrides the environment pattern, advanced organization features and possible plan upgrades, and disabling connections with the risk of breaking sign-in. Notes that removing all methods reverts the organization to environment defaults, and that shared connections can be added back while org-level enterprise connections can be re-enabled. Intended for developers, enterprise admins, and product managers."
updated: 2026-08-22
---
...
Some connection options are properties of the connection itself rather than of the organization. **Show 'Sign in with a code' on password page** is one of these. If an organization uses a shared password connection, the option follows whatever is set on that shared connection, and you cannot switch it on for one organization and off for another while both use the same connection. See [Let users sign in with a code instead of a password](/authenticate/authentication-methods/password-authentication/#let-users-sign-in-with-a-code-instead-of-a-password).
