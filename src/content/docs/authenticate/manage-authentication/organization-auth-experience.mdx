...
ai_summary: "Guide to setting custom authentication methods per organization in Kinde for B2B customers. Explains environment-level shared connections versus organization-level enterprise connections, and that some options such as Show Sign in with a code on password page belong to the connection itself, so organizations sharing a password connection share that setting. Links to the dedicated guide for using password and passwordless auth at the same time. Covers adding existing shared connections to an organization, that organization auth fully overrides the environment pattern, advanced organization features and possible plan upgrades, and disabling connections with the risk of breaking sign-in. Notes that removing all methods reverts the organization to environment defaults, and that shared connections can be added back while org-level enterprise connections can be re-enabled. Intended for developers, enterprise admins, and product managers."
updated: 2026-08-22
---
