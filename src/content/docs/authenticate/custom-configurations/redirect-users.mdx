...
You should be using the state parameter already to protect against CSRF attacks. (Link to guide on state param). Essentially it is a random string that you would store in your application, so when you receive the response from Kinde you can validate it matches the one you sent.
