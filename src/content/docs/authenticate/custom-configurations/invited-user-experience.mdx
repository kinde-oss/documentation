...
- **Invitation expired** — shown when a revoked invitation is opened. Revoked invitations look expired to the invitee, not revoked.
- **Invitation code not usable** — shown when a code can't be used, including when it's unknown, expired, or already used. The same user-facing message is shown in these cases (which may read "Invitation already accepted"), so the underlying reason can't be inferred.
