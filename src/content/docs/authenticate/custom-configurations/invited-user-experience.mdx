...
- **Invitation code not usable** — shown when a code can't be used, including when it's unknown, expired, already used, or revoked. The same user-facing message is shown in every case (which may read "Invitation already accepted"), so the underlying reason can't be inferred.
