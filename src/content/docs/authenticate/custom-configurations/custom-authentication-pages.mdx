...
Add the `connectionId` and `loginHint` parameters to the auth URL.

The `loginHint` pre-populates the email for the user, so they can skip entering their identity. It also tells Kinde where to send their one-time password for passwordless verification.
...
The `loginHint` pre-populates the phone number for the user, so they can skip entering it. It also tells Kinde where to send their one-time password for passwordless verification.

The `loginHint` must use one of these formats: `phone:<intl_number>:<country_code>` or `phone:<+intl_number>:<country_code>`. The `+` symbol is optional as long as the country code is included.
...
- The screen where users can choose to create an account if one was not found
    ...
    <img src="https://imagedelivery.net/skPPZTHzSlcslvHjesZQcQ/96ec47b2-ec12-48b2-2c1c-ca65f15a3900/socialsharingimage" alt="kinde custom sign-in otp verification screen" width="400px" />
